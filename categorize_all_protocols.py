...
def fetch_all_configs_parallel(sources_dict):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        future_to_url = {executor.submit(requests.get, url, timeout=120): url for url in sources_dict.values()}
        ...
        for future in concurrent.futures.as_completed(future_to_url):
            try:
                ...
                if response.status_code == 200 and response.text:
                    ...
                    name = [k for k, v in sources_dict.items() if v == url][0]
...
def main():
    ...
    for config, latency in live_results:
        ...
        if not is_excluded:
            filtered_live_results.append((config, latency))
            
    # مرتب‌سازی لیست فیلتر شده بر اساس پینگ
    filtered_live_results.sort(key=lambda x: x[1])
    
    # استخراج لینک کانفیگ‌ها برای لیست نهایی
    top_200_configs = [res[0] for res in filtered_live_results[:200]]
